...
saveCSVs <- function(dir, constituents) {
  ...
  for (i in constituents) {
   ...
   write_csv(data_xy, paste0("Datasets/wqp_Constituents/CSVs/wqp_", i, "_indexed", ".csv"))
  }
...
saveWQPSQLite <- function(dir) {
  ...
  All_data <- read_csv("Datasets/wqp_Constituents/CSVs/wqp_All_indexed.csv") %>% left_join(coverageInfo, by="COMID")
  All_data$date <- as.character(All_data$date)
  All_data$date_time <- as.character(All_data$date_time)
  file <- "Datasets/wqp_Constituents/wqp_All_indexed.sqlite"
  db <- src_sqlite(file, create=T)
  copy_to(db, All_data, temporary = F)

  secchi_data <- read_csv("Datasets/wqp_Constituents/CSVs/wqp_secchi_indexed.csv") %>% left_join(coverageInfo, by="COMID")
  secchi_data$date <- as.character(secchi_data$date)
  secchi_data$date_time <- as.character(secchi_data$date_time)
  file <- "Datasets/wqp_Constituents/wqp_secchi_indexed.sqlite"
  db <- src_sqlite(file, create=T)
  copy_to(db, secchi_data, temporary = F)

  doc_data <- read_csv("Datasets/wqp_Constituents/CSVs/wqp_doc_indexed.csv") %>% left_join(coverageInfo, by="COMID")
  doc_data$date <- as.character(doc_data$date)
  doc_data$date_time <- as.character(doc_data$date_time)
  file <- "Datasets/wqp_Constituents/wqp_doc_indexed.sqlite"
  db <- src_sqlite(file, create=T)
  copy_to(db, doc_data, temporary = F)
  
  chlorophyll_data <- read_csv("Datasets/wqp_Constituents/CSVs/wqp_chlorophyll_indexed.csv") %>% left_join(coverageInfo, by="COMID")
  ...
  file <- "Datasets/wqp_Constituents/wqp_chlorophyll_indexed.sqlite"
  db <- src_sqlite(file, create=T)
  copy_to(db, chlorophyll_data, temporary = F)
  
  tss_data <- read_csv("Datasets/wqp_Constituents/CSVs/wqp_tss_indexed.csv") %>% left_join(coverageInfo, by="COMID")
  tss_data$date <- as.character(tss_data$date)
  tss_data$date_time <- as.character(tss_data$date_time)
  file <- "Datasets/wqp_Constituents/wqp_tss_indexed.sqlite"
  db <- src_sqlite(file, create=T)
